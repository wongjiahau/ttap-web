...
describe("DefilterTimetable action", () => {
  ...
  it("should set property of FiltrateTimetables and ResidueTimetables ", () => {
    ...
    const newState1 = MasterStateReducer(
      initialState,
      new FilterTimetable(state0)
    );
    const newState2 = MasterStateReducer(
      newState1,
      new DefilterTimetable(state1)
    );
    expect(newState2.TimetableListState.FiltrateTimetables.length).to.eq(
      initialState.TimetableListState.FiltrateTimetables.length
    );
  ...
  it("should set property of TotalState based on the filtered timetables", () => {
    ...
    expect(newState3.SetTimeConstraintState.TotalMatrix).to.deep.eq(
      newState1.SetTimeConstraintState.TotalMatrix
    );
  });
  ...
  it("should set property of ClickedTimeConstraint", () => {
    ...
    expect(newState1.SetTimeConstraintState.ClickedTimeConstraint).to.deep.eq([
      parseInt("10000", 2),
      0,
      0,
      0,
    ...
    expect(newState2.SetTimeConstraintState.ClickedTimeConstraint).to.deep.eq([
      0, 0, 0, 0, 0, 0, 0,
    ]);
  });
