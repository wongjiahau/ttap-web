...
describe("searchSubjectList action", () => {
    it("should render all subject to be visible if searched text is empty", () => {
        ...
        const newState = SubjectListStateReducer(initialState, new SearchSubjectList("").Action());
    ...
    it("should make some subject invisible based on if the searched text match only some subejcts", () => {
        const initialState = new SubjectListState(GetTestSubjects1());
        const newState = SubjectListStateReducer(initialState, new SearchSubjectList("hubungan etnik").Action());
    ...
    it("should toggle IsShowSelectedSubjectsOnly to false when dispatched", () => {
        ...
        const newState = SubjectListStateReducer(initialState, new SearchSubjectList("lol random stuff").Action());
    ...
    it("should make all subject visible when the searched string is empty again", () => {
        const initialState = new SubjectListState(GetTestSubjects1());
        let newState = SubjectListStateReducer(initialState, new SearchSubjectList("hubungan etnik").Action());
        newState = SubjectListStateReducer(newState, new SearchSubjectList("").Action());
        expect(newState.Subjects.filter((s) => s.IsVisible).length)
            .to
    ...
    it("should set the SearchedText based on input", () => {
        const initialState = new SubjectListState(GetTestSubjects1());
        const newState = SubjectListStateReducer(initialState, new SearchSubjectList("spongebob").Action());
    ...
    it("should allow user to search using subjects initial", () => {
        ...
        const newState = SubjectListStateReducer(initialState, new SearchSubjectList(searchText).Action());
        expect(newState.Subjects.filter((s) => s.IsVisible).length)
            .to
            .eq(1);
    });
