...
describe([
    ...
    it("property test", () => {
        ...
        state = MasterStateReducer(state, new ToggleSubjectSelection(IndexOf.ITBS));
        expect(state.TimetableListState.SlotViewModelStore.GetAll()).to.have.lengthOf(95);
    ...
    it("specific scenario 1", () => {
        ...
        const state0 = MasterStateReducer(intialState, new ToggleSubjectSelection(IndexOf.FM2));
        let state1 = MasterStateReducer(state0, new ToggleSubjectSelection(IndexOf.ITBS));
        state1 = MasterStateReducer(state1, new FindAlternativeSlotsOfCurrentSlots())

        const findSlot = (state: IMasterState, targetSlot: {
            SubjectCode: string
            Type: string
            Group: string
        }): ISlotViewModel => {
        ...
        const FM2_P1 = findSlot(state1, { SubjectCode: CodeOf.FM2, Type: "P", Group: "1" })
        const FM2_P14a = findSlot(state1, { SubjectCode: CodeOf.FM2, Type: "P", Group: "14" })
        ...
        const ITBS_T1 = findSlot(state2, { SubjectCode: CodeOf.ITBS, Type: "T", Group: "1" })
        const ITBS_T5 = findSlot(state2, { SubjectCode: CodeOf.ITBS, Type: "T", Group: "5" })
        ...
        const FM2_P14b = findSlot(state3, { SubjectCode: CodeOf.FM2, Type: "P", Group: "14" })
        const FM2_P8 = findSlot(state3, { SubjectCode: CodeOf.FM2, Type: "P", Group: "8" })
...
const getCurrentTimetableSlots = (state: IMasterState) => {
    const slotUids = state.TimetableListState.FiltrateTimetables
...
const expectOnlyDiffersBySourceSlot = ({
    ...
    const nextState = MasterStateReducer(previousState, new GoToThisAlternativeSlot(sourceSlot.Uid, destinationSlotUid))
    ...
    expect(destinationTimetableSlots.sort((a, b) => a.Uid - b.Uid))
    ...
    expect(removeCurrentSlotKind(destinationTimetableSlots))
    ...
    expect(destinationTimetableSlots.some(slot => slot.Uid === sourceSlot.Uid)).to.eq(false)

    // Expect the destination timetable should contain the destination slot
    expect(destinationTimetableSlots.some(slot => slot.Uid === destinationSlotUid)).to.eq(true)


    return MasterStateReducer(nextState, new FindAlternativeSlotsOfCurrentSlots())
}
