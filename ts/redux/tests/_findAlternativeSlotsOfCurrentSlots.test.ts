...
describe("find alternative slots of current slot", () => {
  it("should not show alternative slots if not all sibling slots are shown", () => {
    ...
    const slotsToBeClicked =
      state.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.Type === "L" && x.SubjectCode === "UEME2123" && x.Group[0] === "1"
      )[0]; // Fluid Mechanic I, Lecture 1

    state = MasterStateReducer(state, new ShowAlternateSlot(slotsToBeClicked));

    const slot = state.TimetableListState.ShowingAlternateSlotOf;
    if (!slot) {
      throw new Error(`slot should be defined`);
    }
    expect(slot.AlternativeSlots.some((x) => x.slot.Group[0] === "2")).to.eq(
      false
    );
    expect(
      state.TimetableListState.SlotViewModelStore.GetAll()
