...
describe("goToPrevTimetable action", () => {
    ...
    it("should decrement the CurrentIndex and reset SubCurrentIndex", () => {
        const initialState = getInitialState([NullGroupedTimetable, NullGroupedTimetable]);
        ...
        const newState2 = MasterStateReducer(newState, new GoToPreviousSubTimetable());
    ...
    it("decrement the CurrentIndex cyclically", () => {
        const initialState = getInitialState([NullGroupedTimetable, NullGroupedTimetable, NullGroupedTimetable]);
        const newState = MasterStateReducer(initialState, new GoToPrevTimetable());
