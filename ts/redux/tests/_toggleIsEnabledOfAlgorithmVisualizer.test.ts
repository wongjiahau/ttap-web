...
describe("ToggleIsOpenOfSummary action", () => {
    it("'s typename should be 'toggle is enabled of algorithm visualizer'", () => {
        ...
        expect(action.TypeName()).to.eq("toggle is enabled of algorithm visualizer");
    });
    ...
    it("should toggle IsEnabled if pass in nothing", () => {
        ...
        const newState = MasterStateReducer(initialState, new ToggleIsEnabledOfAlgorithmVisualizer());
        expect(newState.AlgorithmVisualizerState.isEnabled).to.eq(true);
        const newState1 = MasterStateReducer(newState, new ToggleIsEnabledOfAlgorithmVisualizer());
    ...
    it("should set IsEnabled to false if passed in false", () => {
        ...
        let newState = MasterStateReducer(initialState, new ToggleIsEnabledOfAlgorithmVisualizer(true));
        expect(newState.AlgorithmVisualizerState.isEnabled).to.eq(true);
        newState = MasterStateReducer(newState, new ToggleIsEnabledOfAlgorithmVisualizer(false));
