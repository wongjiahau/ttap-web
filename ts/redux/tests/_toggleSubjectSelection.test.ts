...
describe("toggle subject selection action", () => {
  it("'s type name should be 'select subject'", () => {
    ...
    expect(action.TypeName()).to.eq("toggle subject selection");
  });
  ...
  it("should toggle selection on a subject based on its subject index", () => {
    ...
    expect(newState.SubjectListState.Subjects[IndexOf.HE].IsSelected).to.eq(
      true
    );
  });
  ...
  it("should show all subjects when user deselected all subjects", () => {
    ...
    expect(newState.SubjectListState.Subjects.every((x) => x.IsVisible)).to.eq(
      true
    );
  });
  ...
  describe("behaviour of subject selection", () => {
    it("case 1", () => {
      ...
      const clashReport =
        newState.SubjectListState.Subjects[IndexOf.BKA].ClashReport;
      if (!clashReport) {
        throw new Error();
      }
    ...
    it("case 2", () => {
      ...
      newState = MasterStateReducer(
        newState,
        new ToggleSubjectSelection(IndexOf.BMK2)
      );
      newState = MasterStateReducer(
        newState,
      ...
      const clashReport =
        newState.SubjectListState.Subjects[IndexOf.BEAM].ClashReport;
      if (!clashReport) {
        throw new Error();
      }
  ...
  describe("behaviour of subject deselection", () => {
    it("case 1", () => {
      ...
      expect(newState.SubjectListState.Subjects[IndexOf.BKA].ClashReport).to.eq(
        null
      );
    });
    ...
    it("case 2", () => {
      ...
      expect(clashReport.TargetName).to.eq("Bahasa Melayu Komunikasi 2");
    });
