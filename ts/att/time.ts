...
export class Time {
  ...
  public static CreateTime12Hour(
    hour: number,
    minute: number,
    isPm: boolean
  ): Time {
    if (hour < 1 || hour > 12) {
      throw new RangeError(
        "Expected value is between 1 to 12, but actual is {hour}"
      );
    }
    if (isPm && hour !== 12) {
      hour += 12;
  ...
  public static CreateTimeFrom12Hour_2(
    hour: number,
    minute: number,
    isPm: boolean
  ): Time {
  ...
  public Minus(other: Time): TimeSpan {
    if (this.LessThan(other)) {
      throw new RangeError(
        `${this.To12HourFormat(true)} is less than ${other.To12HourFormat(
          true
        )}, so a cannot be subtracted by b`
      );
    }
