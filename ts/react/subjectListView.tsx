...
export interface ISubjectListViewProps
  extends ISubjectListViewStateProps,
    ISubjectListViewDispatchProps {}

export class SubjectListView extends React.Component<
  ISubjectListViewProps,
  {
...
> {
  ...
  public render() {
    ...
    const showErrorMessage =
      this.props.Subjects.filter((x) => x.IsVisible).length === 0;

    const numberOfSelectedSubjects = this.props.Subjects.filter(
      (s) => s.IsSelected
    ).length;
        ...
        <section onKeyUp={this.checkKeys} style={this.state.sectionStyle}>
          ...
          <Paper style={divStyle}>
            <div id="subject-list-container">
              {!showErrorMessage ? subjectViews : errorMessage}
            </div>
          </Paper>
  ...
  private Focus = (where: "previous" | "next"): void => {
    const idOfFocusedSubjectView = document.activeElement
      ? document.activeElement.id
      : "";
    const subjectViews = document.getElementsByClassName(
      "subjectview"
