...
describe("GenerateSubjectSchema", () => {
  it("should throw error if some slots are from different subjects", () => {
    ...
    expect(() => {
      GenerateSubjectSchema(
        MockRawSlotStore.GetBunch(acp.SlotUids.concat(he.SlotUids))
      );
    }).to.throw();
  });
  ...
  it("case 1", () => {
    const subjects = GetTestSubjects1();
    const acp = subjects[IndexOf.ACP];
    const result = GenerateSubjectSchema(
      MockRawSlotStore.GetBunch(acp.SlotUids)
    );
  ...
  it("case 2", () => {
    ...
    const result = GenerateSubjectSchema(
      MockRawSlotStore.GetBunch(beam.SlotUids)
    );
    expect(result.GotLecture).to.eq(true);
  ...
  it("case 3", () => {
    ...
    const result = GenerateSubjectSchema(
      concreteTechnology.SlotUids.map(
        (uid) => rawSlots.filter((s) => s.Uid === uid)[0]
      )
    );
    expect(result.GotLecture).to.eq(true);
    expect(result.GotTutorial).to.eq(true);
