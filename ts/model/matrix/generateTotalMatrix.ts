...
export function GenerateTotalMatrix(timetables: IGroupedTimetable[], uidsOfClickedMatrix: string[] = []): STCBox[] {
    ...
    for (let day = 0; day < 7; day++) {
        ...
        const dus = DecToBin(definitelyUnoccupiedMatrix[day], totalNumberOfHalfHourPerDay).split("").reverse().join("");
...
export function GetMaybeOccupiedMatrix(definitelyOccupiedMatrix: number[], definitelyUnoccupiedMatrix: number[]): number[ /*7*/ ] {
