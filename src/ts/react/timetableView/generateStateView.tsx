...
export function GenerateStateViews(
    ...
    switch (state.Kind) {
      ...
      case BoxKind.MaybeOccupied:
        return (
          <MaybeOccupiedBox
            handleClick={() => {
              handleSetTimeContraintAt(state);
            }}
          />
        );
      case BoxKind.Clicked:
        return (
          <ClickedBox
            handleClick={() => {
              handleDesetTimeConstraintAt(state);
            }}
          />
        );
      default:
  ...
  states.forEach((s) => {
    ...
    const key = s.Uid.toString();
    child.push(<div key={key}>{content}</div>);
    layouts.push({
      i: key,
...
const MaybeOccupiedBox = (props: { handleClick: () => void }) => {
  const style: React.CSSProperties = {
    ...boxFrameStyle,
  ...
  const html = (
    <span style={{ fontSize: "14px" }}>
      Click me if you don't want to have class here
    </span>
  );
  return (
...
const ClickedBox = (props: { handleClick: () => void }) => {
  const style: React.CSSProperties = {
    ...boxFrameStyle,
