...
export class SlotView extends React.Component<ISlotViewProps, ISlotViewState> {
  ...
  public render() {
    ...
    if (slot.SourceSlotUid !== undefined) {
      // add border glow
      slotStyle = {
        ...slotStyle,
        color: "white",
    ...
    const clickHandler = (event: React.MouseEvent<HTMLDivElement>) => {
      if (
        handleShowAlternateSlot !== undefined &&
        slot.AlternativeSlots.length > 0
      ) {
      ...
      if (
        handleGoToThisAlternateSlot !== undefined &&
        slot.SourceSlotUid !== undefined
      ) {
    ...
    const venue = `${slot.Room[slot.CurrentChoice]}${
      slot.ClassMode ? ` [${[slot.ClassMode]}]` : ""
    }`;
    ...
    return (
      <Tooltip
        arrow={true}
        position="left"
        html={tooltipTitle(slot, isShowingAlternativeSlotsOfThisSlot)}
      >
        <div
          className={className}
          style={slotStyle}
                ...
                isShowingAlternativeSlotsOfThisSlot
                  ? this.props.alternativeSlots.map((alternativeSlot) => ({
                      ...
                      ...(ParseDay(alternativeSlot.Day) > ParseDay(slot.Day)
                        ...
                        : TimePeriod.Parse(alternativeSlot.TimePeriod).StartTime
                            .Hour >
...
function tooltipTitle(
  s: ISlotViewModel,
  isShowingAlternativeSlotOfThisSlot: boolean
) {
  ...
  return (
    <div style={style}>
      ...
      <br />[{s.SubjectCode}]
      <br />{" "}
      {s.SourceSlotUid !== undefined
        ? "(Click to go to this slot)"
        : s.AlternativeSlots.length === 0
        ? ""
        : isShowingAlternativeSlotOfThisSlot
        ? "(Click to hide alternative slots)"
...
export function getSlotContent(slot: ISlotViewModel) {
  return `${GetInitial(slot.SubjectName)} - ${slot.Type}(${
    slot.Group[slot.CurrentChoice]
  })`;
}
