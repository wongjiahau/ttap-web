...
export interface ISlotsTableViewProps
  extends ISlotsTableViewStateProps,
...
export class SlotsTable extends React.Component<
  ISlotsTableViewProps,
    ...
    return (
      ...
      >
        <section style={this.state.sectionStyle}>
          <header style={headerStyle}>
            <StackPanel orientation="vertical">
              <Typography variant="display1" color="primary">
                Below are the time slots of selected subjects.
              </Typography>
              <Typography variant="subheading" gutterBottom={true}>
                You can select or deselect some specific time slots.
              </Typography>
            </StackPanel>
          </header>
          <Paper style={divStyle}>
            {this.props.selectedSubjects.map((subject) => {
              return (
                    ...
                    <TableHead>
                      <TableRow>
                        <TableCell padding="checkbox">
                          <Checkbox
                            indeterminate={
                              this.props.subjectStates[subject.Code] ===
                              "intermediate"
                            }
                            checked={
                              this.props.subjectStates[subject.Code] === "true"
                            }
                            onClick={() =>
                              this.props.handleSlotsGroupCheckChanged(
                                subject.Code
                              )
                            }
                          />
                    ...
                    <TableBody>
                      {this.props.rawSlotStore
                        ...
                        .map((slot, index) => {
                          ...
                          const clickHandler = () =>
                            this.props.handleSlotCheckChanged(
                              slot.Number,
                              checked,
                              subject.Code
                            );
          ...
          <div>{GenerateErrorLabels(this.props.errorMessages)}</div>
  ...
  if (diffReports[0].MissingSlotType === "no possible timetables found") {
    return (
      <div style={errorStyle}>
        <ul>
          <li>
            The currently selected slots do not produce any possible timetables.
  ...
  return (
    <div style={errorStyle}>
      <ul>
        {diffReports.map((r) => (
          <li>
            At least one {getType(r.MissingSlotType)}&nbsp;is needed for{" "}
            {Subject.GetSubjectNameOf(r.SubjectCode)}
          </li>
        ))}
      </ul>
    </div>
