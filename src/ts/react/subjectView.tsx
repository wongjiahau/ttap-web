  ...
  public render() {
    ...
    return (
      <div className="ui pushable" style={{ overflowY: "hidden" }}>
        <div
          style={clashReportStyle}
          className={
            "ui " + (gotClashReport ? "visible" : "") + " right overlay sidebar"
          }
        >
          {getClashReport(this.props.clashReport)}
        </div>
        <div className={(gotClashReport ? "dimmed" : "") + " pusher"}>
          <div style={divStyle}>
            <ListItem
              id={this.props.id}
...
const getClashReport = (x: ClashReport | null) => {
  if (x) {
    if (x.Type === "single") {
      return (
        <div>
          Cannot select this subject as it clashes with : <br />
          <b>{x.TargetName}</b>
        </div>
      );
      ...
      return (
        <div>
          Cannot select this subject as it clashes with the subject(s) you
          selected previously.
        </div>
      );
    }
