import { Defilter } from "../../model/matrix/defilter";
import { GenerateTotalMatrix } from "../../model/matrix/generateTotalMatrix";
...
export class DefilterTimetable extends MasterStateAction {
  ...
  protected GenerateNewState(state: IMasterState): IMasterState {
    ...
    const newUidsOfClickedBox =
      state.SetTimeConstraintState.UidsOfClickedBoxes.filter(
        (x) => x !== this.clickedBox.Uid
      );
    const [rescued, unrescued] = Defilter(
      state.TimetableListState.ResidueTimetables,
      newClickedTimeConstraint
    );
    const newFiltrateTimetables =
      state.TimetableListState.FiltrateTimetables.concat(rescued);
    ...
    return {
      ...
      SetTimeConstraintState: {
        ...
        TotalMatrix: GenerateTotalMatrix(
