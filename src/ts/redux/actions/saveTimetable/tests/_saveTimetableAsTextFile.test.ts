...
describe("findSisterSlots", () => {
  it("case 1", () => {
    ...
    const slotsOfCurrentTimetable: IRawSlot[] = [
      // Fluid Mechanics I
      allSlots.filter(
        (s) => s.SubjectCode === "UEME2123" && s.Type === "L" && s.Group === "3"
      )[0],
      allSlots.filter(
        (s) =>
          s.SubjectCode === "UEME2123" && s.Type === "T" && s.Group === "13"
      )[0],
      allSlots.filter(
        (s) => s.SubjectCode === "UEME2123" && s.Type === "P" && s.Group === "1"
      )[0],

      // Fluid Mechanics II
      allSlots.filter(
        (s) => s.SubjectCode === "UEME3112" && s.Type === "L" && s.Group === "1"
      )[0],
      allSlots.filter(
        (s) => s.SubjectCode === "UEME3112" && s.Type === "T" && s.Group === "5"
      )[0],
      allSlots.filter(
        (s) =>
          s.SubjectCode === "UEME3112" && s.Type === "P" && s.Group === "10"
      )[0],
    ];
