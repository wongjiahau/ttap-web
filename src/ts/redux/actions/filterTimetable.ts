import { Filter } from "../../model/matrix/filter";
...
export class FilterTimetable extends MasterStateAction {
  ...
  protected GenerateNewState(state: IMasterState): IMasterState {
    ...
    const newUidsOfClickedState =
      state.SetTimeConstraintState.UidsOfClickedBoxes.concat(
        this.clickedState.Uid
      );
    const newClickedTimeConstraint =
      state.SetTimeConstraintState.ClickedTimeConstraint.slice();
    newClickedTimeConstraint[this.clickedState.Day] |=
      this.clickedState.TimePeriod;
    return {
      ...state,
      TimetableListState: {
        ...
        ResidueTimetables:
          state.TimetableListState.ResidueTimetables.concat(residue),
      },
