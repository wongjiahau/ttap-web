...
describe("searchSubjectList action", () => {
  it("should render all subject to be visible if searched text is empty", () => {
    ...
    const newState = MasterStateReducer(
      initialState,
      new SearchSubjectList("")
    );
    expect(newState.SubjectListState.Subjects.every((s) => s.IsVisible)).to.eq(
      true
    );
  });
  ...
  it("should make some subject invisible based on if the searched text match only some subejcts", () => {
    ...
    const newState = MasterStateReducer(
      initialState,
      new SearchSubjectList("hubungan etnik")
    );
    expect(
      newState.SubjectListState.Subjects.filter((s) => s.IsVisible).length
    ).to.eq(1);
  });
  ...
  it("should toggle IsShowSelectedSubjectsOnly to false when dispatched", () => {
    ...
    const newState = MasterStateReducer(
      initialState,
      new SearchSubjectList("lol random stuff")
    );
  ...
  it("should make all subject visible when the searched string is empty again", () => {
    ...
    expect(
      newState.SubjectListState.Subjects.filter((s) => s.IsVisible).length
    ).to.eq(newState.SubjectListState.Subjects.length);
  });
  ...
  it("should set the SearchedText based on input", () => {
    ...
    const newState = MasterStateReducer(
      initialState,
      new SearchSubjectList("spongebob")
    );
  ...
  it("should allow user to search using subjects initial", () => {
    ...
    const newState = MasterStateReducer(
      initialState,
      new SearchSubjectList(searchText)
    );
    expect(
      newState.SubjectListState.Subjects.filter((s) => s.IsVisible).length
    ).to.eq(1);
