...
describe("UpdateTotalState action", () => {
  ...
  it("should clear UidsOfClickedState", () => {
    ...
    expect(newState.SetTimeConstraintState.UidsOfClickedBoxes).to.deep.eq([
      "05",
    ]);
    newState = MasterStateReducer(newState, new UpdateTotalMatrix());
    expect(newState.SetTimeConstraintState.UidsOfClickedBoxes).to.have.lengthOf(
      0
    );
  });
