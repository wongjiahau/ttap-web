...
describe("showAlternateSlot action", () => {
  it("should set alternate slots based on clicked slot", () => {
    ...
    const newState0 = MasterStateReducer(
      initialState,
    ...
    let newState2 = MasterStateReducer(
      newState1,
      new ToggleSubjectSelection(IndexOf.ASSD)
    );
    newState2 = MasterStateReducer(
      newState2,
      new FindAlternativeSlotsOfCurrentSlots()
    );
    const slotsToBeClicked =
      newState2.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.Type === "T" && x.SubjectCode === "UEME2123" && x.Group[0] === "3"
      )[0]; // Fluid Mechanic I, Tutorial 3

    const newState3 = MasterStateReducer(
      newState2,
      new ShowAlternateSlot(slotsToBeClicked)
    );
    if (!newState3.TimetableListState.ShowingAlternateSlotOf) {
      throw new Error();
    }
    expect(newState3.TimetableListState.ShowingAlternateSlotOf.Uid).to.deep.eq(
      slotsToBeClicked.Uid
    );

    const alternativeSlots3 = newState3.TimetableListState
      .ShowingAlternateSlotOf
      ? newState3.TimetableListState.ShowingAlternateSlotOf.AlternativeSlots
      : [];
    expect(alternativeSlots3).to.have.lengthOf(13);
    expect(alternativeSlots3.map((x) => x.slot.Group)).to.deep.eq([
      ["4"],
      ["9"],
      ["10"],
      ["11"],
    ...
    const newState4 = MasterStateReducer(
      newState3,
      new ShowAlternateSlot(slotsToBeClicked)
    );

    const alternativeSlots4 = newState4.TimetableListState
      .ShowingAlternateSlotOf
    ...
    // case 2
    const slotsToBeClicked2 =
      newState2.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.Type === "T" && x.SubjectCode === "UEMX4313" && x.Group[0] === "1"
      )[0]; // ASSD, Tutorial 1
    const newState5 = MasterStateReducer(
      newState4,
      new ShowAlternateSlot(slotsToBeClicked2)
    );

    const alternativeSlots5 = newState5.TimetableListState
      .ShowingAlternateSlotOf
  ...
  it("should not show alternate slots that is filtered out by set time constraint", () => {
    ...
    const initialState = GetMockInitialState("heng_2017_apr");
    const state0 = MasterStateReducers(initialState, [
      new ToggleSubjectSelection(IndexOf.ASSD),
      new FindAlternativeSlotsOfCurrentSlots(),
    ]);

    const slotsToBeClicked1 =
      state0.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.SubjectCode === "UEMX4313" && x.Type === "T" && x.Group[0] === "1"
      )[0]; // ASSD I, Tutorial 1

    const state1 = MasterStateReducer(
      state0,
      new ShowAlternateSlot(slotsToBeClicked1)
    );
    ...
    const greenBoxToBeClicked = new STCBox(
      BoxKind.MaybeOccupied,
      1,
      parseInt("10000", 2),
      4
    ); // Tuesday 10.30am to 11.00am
    ...
    const slotsToBeClicked2 =
      state2.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.SubjectCode === "UEMX4313" && x.Type === "T" && x.Group[0] === "1"
      )[0]; // ASSD I, Tutorial 1
    ...
    const alternativeSlots2 = newState2.TimetableListState
      .ShowingAlternateSlotOf
    ...
    expect(alternativeSlots1.length > alternativeSlots2.length).to.eq(true);
  });
  ...
  it(
    ...
    () => {
      ...
      const slotsToBeClicked =
        newState1.TimetableListState.SlotViewModelStore.GetAll().filter(
          (x) =>
            x.Type === "T" && x.SubjectCode === "UEME2123" && x.Group[0] === "3"
        )[0]; // Fluid Mechanic I, Tutorial 3
      ...
      expect(newState2.TimetableListState.ShowingAlternateSlotOf).to.not.eq(
        null
      );

      expect(
        MasterStateReducer(newState2, new GoToNextTimetable())
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);

      expect(
        MasterStateReducer(newState2, new GoToPrevTimetable())
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);

      expect(
        MasterStateReducer(newState2, new GoToRandomTimetable())
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);

      expect(
        MasterStateReducer(newState2, new GoToNextSubTimetable())
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);

      expect(
        MasterStateReducer(newState2, new GoToPreviousSubTimetable())
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);

      expect(
        MasterStateReducer(newState2, new ToggleSetTimeConstraintView(true))
          .TimetableListState.ShowingAlternateSlotOf
      ).to.eq(null);
    }
  );
  ...
  it("should not show the alternative slots of other slot if ShowingAlternateSlotOf is not null", () => {
    ...
    const slotsToBeClicked1 =
      state1.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.Type === "T" && x.SubjectCode === "UEME2123" && x.Group[0] === "3"
      )[0]; // Fluid Mechanic I, Tutorial 3

    const state2 = MasterStateReducer(
      state1,
      new ShowAlternateSlot(slotsToBeClicked1)
    );
    ...
    const slotsToBeClicked2 =
      state2.TimetableListState.SlotViewModelStore.GetAll().filter(
        (x) =>
          x.Type === "P" && x.SubjectCode === "UEME2123" && x.Group[0] === "1"
      )[0]; // Fluid Mechanic I, Practical 1

    const state3 = MasterStateReducer(
      state2,
      new ShowAlternateSlot(slotsToBeClicked2)
    );
