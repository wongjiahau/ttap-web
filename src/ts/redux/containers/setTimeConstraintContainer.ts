...
const mapStateToProps = (state: any): ISetTimeConstraintViewStateProps => {
  const stcState = state.MasterStateReducer
    .SetTimeConstraintState as ISetTimeConstraintState;
...
const mapDispatchToProps = (
  ...
  return {
    ...
    handleDesetTimeConstraintAt: (stcBox: STCBox) =>
      dispatch(new DefilterTimetable(stcBox)),
    handleSetTimeConstraintAt: (stcBox: STCBox) =>
      dispatch(new FilterTimetable(stcBox)),
  };
};

export const SetTimeConstraintContainer = connect(
  mapStateToProps,
  mapDispatchToProps
)(SetTimeConstraintView);
